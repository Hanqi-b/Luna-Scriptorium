...
翻译分两步准备：先检查原文 Markdown，再自动规划并锁定章节和分块。运行时由主 agent 调度四个 GPT-6 Luna Max 翻译 agent，记录进度以便中断后恢复。使用方法和运行约束见 [`translate-book/SKILL.md`](translate-book/SKILL.md)；验证范围及限制见 [`VALIDATION.md`](VALIDATION.md)。
...
ln -s "$(pwd)/translate-book" "$HOME/.agents/skills/translate-book"
```
